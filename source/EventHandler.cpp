...
ButtonState EventHandler::GetKeyState(SDL_Keycode key) const {
	ButtonState state;
	// TODO-DG: Get rid of these try-catch statements. They're super unnecessary.
	try {
...
bool EventHandler::IsKeyDown(SDL_Keycode key) const { return GetKeyState(key) == ButtonState::Down; }
...
bool EventHandler::IsMouseButtonDown(MouseButton button) const { return GetMouseButtonState(button) == ButtonState::Down; }

SDL_Point EventHandler::GetMousePoint() const { return mousePoint; }

void EventHandler::AddKeyboardEvent(const SDL_Event &event) { events.push_back(CreateKeyboardEvent(event)); }
